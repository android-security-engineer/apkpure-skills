...
var DEFAULT_DOWNLOAD_DIR = join(homedir(), ".apkpure", "downloads");
var DEFAULT_CONFIG = {
  mode: "android",
  locale: "en-US",
  timeout: 3e4,
...
function makeMobileHeaders() {
  ...
  cachedHeaders = {
    ...
    "Ual-Access-Sequence": randomUUID2(),
...
async function mobileRequest(url, options = {}) {
  const dir = mkdtempSync(join2(tmpdir(), "apkpure-mobile-"));
  const output = join2(dir, "response.bin");
  try {
    const env = {
      ...process.env,
...
import { mkdtempSync as mkdtempSync2, readFileSync as readFileSync2, rmSync as rmSync2, statSync, mkdirSync as mkdirSync2 } from "fs";
...
async function requestViaCurlCffi(py, url, outFile, opts) {
  ...
  const body = readFileSync2(outFile);
...
async function requestViaBinary(bin, url, outFile, opts) {
  ...
  let headers = {};
  try {
    headers = parseHeaderDump(readFileSync2(headerDump, "utf-8"));
  } catch {
  } finally {
    try {
      rmSync2(headerDump, { force: true });
    } catch {
    }
  }
  const body = readFileSync2(outFile);
...
async function impersonateRequest(url, opts = {}) {
  ...
  const dir = mkdtempSync2(join4(tmpdir2(), "apkpure-imp-"));
  const outFile = join4(dir, "resp.bin");
  try {
...
import { mkdirSync as mkdirSync3, renameSync, statSync as statSync2, unlinkSync, existsSync } from "fs";
...
async function downloadApk(url, packageName, version, fileType, options, proxy = "") {
  ...
  const filePath = join5(options.outputDir, fileName);
...
var CLASH_CONFIG_PATHS = [
  ...
  join6(homedir2(), ".config/clash-verge"),
  join6(homedir2(), ".config/clash"),
  join6(homedir2(), ".config/mihomo")
];
...
function extractPortFromClashConfig(configDir) {
  ...
  for (const file of configFiles) {
    const filePath = join6(configDir, file);
...
async function detectProxy() {
  ...
  for (const configDir of CLASH_CONFIG_PATHS) {
    if (!existsSync2(configDir)) continue;
